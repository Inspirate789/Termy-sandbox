...
import (
	"git.iu7.bmstu.ru/sam20u745/testing-web-2023-labs/internal/adapters/server/errors"
...
func (s *Server) getUnits(ctx *gin.Context) {
	...
	unitsDTO, err := s.storageService.GetUnits(layer)
	if err != nil {
		_ = ctx.AbortWithError(http.StatusInternalServerError, err)
		return
	}
...
//	@Failure		500	{object}	string
//	@Router			/units [delete]
func (s *Server) deleteUnit(ctx *gin.Context) {
	layer := ctx.Query("layer")
	...
	err = s.storageService.DeleteUnit(layer, unitDTO)
	if err != nil {
		_ = ctx.AbortWithError(http.StatusInternalServerError, err)
		return
	}
