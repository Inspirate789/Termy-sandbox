...
func (r *UnitsPgRepository) makeOutputUnitDTO(layer, lang string, unit entities.Unit) (interfaces.OutputUnitDTO, error) {
	...
	propertiesID, contextsID := make([]int, 0), make([]int, 0)
...
func (r *UnitsPgRepository) getUnitsByQueries(layer string, langs []string, qData unitQueries) (joinedUnitMaps, entities.UnitsMap, error) {
	...
	for _, lang := range langs {
		args1["lang"] = lang
		unlinkedUnits := make([]entities.Unit, 0)
...
func (r *UnitsPgRepository) GetAllUnits(layer string) (interfaces.OutputUnitsDTO, error) {
	...
	err = sqlx_utils.Select(context.Background(), r.conn, &contexts, selectContextsById, pq.Array(contextsID))
	if err != nil {
		return interfaces.OutputUnitsDTO{}, err
	}
...
func (r *UnitsPgRepository) insertContext(tx sqlx.ExtContext, ctxText string) (int, error) {
	var id int
	err := sqlx_utils.Get(context.Background(), tx, &id, insertContext, ctxText)
	if err != nil {
		return 0, err
	}
	return id, nil
...
func (r *UnitsPgRepository) DeleteUnit(layer, lang, unitName string) error {
	args := map[string]any{
		"layer_name": layer,
		"lang":       lang,
		"unit_name":  unitName,
	}
	_, err := sqlx_utils.NamedExec(context.Background(), r.conn, deleteUnit, args)
	return err
}
